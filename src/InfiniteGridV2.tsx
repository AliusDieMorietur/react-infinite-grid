...
export type InfiniteGridV2Props<T> = {
  items: T[];
  className?: string;
  columns?: number;
  style?: CSSProperties;
...
export const InfiniteGridV2 = <T,>({
  ...
  const onScroll = (e: any) => {
    ...
    if (halfOfBatchGone) {
      setState((prevState) => ({
        ...prevState,
        visibleStart: prevState.visibleStart + columns * 2,
        visibleEnd: prevState.visibleEnd + columns * 2,
      }));
    }
    const halfBatchBefore = element.scrollTop - topOffset < batchHeight / 2;
    if (halfBatchBefore) {
      setState((prevState) => ({
        ...prevState,
        visibleStart: prevState.visibleStart - columns * 2,
